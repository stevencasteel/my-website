...
const iconWidth = socialIcons[0].offsetWidth + 10; // Width of one icon + margin
const totalIcons = socialIcons.length;

// Clone the social icons for infinite loop
for (let i = 0; i < totalIcons; i++) {
    const clone = socialIcons[i].cloneNode(true);
    socialTrack.appendChild(clone);
}

// Set the width of the track to accommodate all icons (original and cloned)
socialTrack.style.width = `${iconWidth * totalIcons * 2}px`;
...
let x = 0;
const speed = 3; // Adjust for scroll speed

function animateSocial() {
    x -= speed;
    if (x <= -(iconWidth * totalIcons)) {
        x = 0;
    }
    socialTrack.style.transform = `translateX(${x}px)`;
...
document.querySelectorAll('.video-carousel iframe').forEach(iframe => {
